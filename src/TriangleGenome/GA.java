 ...
 *
 * Possible mutations (changing one of the 10 genes of a triangle) aka 
 * pure hill climbing, but through explotation of patters we should be able
 * to have mutations of whole verticies (x,y), colors (a,r,g,b), size,
 * order etc. 
 * 
 * The GA currently does definitely not work, but some possible structure
 * for how it might work is laid out. Currently during the mutations
 * of the GA almost every mutation results in a non improvement in the
 * fitness. If you were to comment the undo function following 
 * generations without improvement you will see the GA continuously 
 * drifting away from the original fitness to something lower (should not happen)
 * Another problem is the time taken per generation which make its difficult
 * to test as it takes a while to run a decent number of generations. 
 * There are two main things that take time during each generation. The first
 * is rendering the triangles to an image (I am currently working on a way
 * to increase the speed via openGL) and the second is the fitness function
 * (which from what I'v heard we will probably have to use something called
 * openCL to increase its speed). 
 */
public class GA extends Stage{
	...
	GA(ArrayList<Triangle> DNA,double initialFitness,Image originalImage,int IMAGE_WIDTH,int IMAGE_HEIGHT)
	{
		...
		//Make sure the BufferedImage type includes alpha since we have to take account for it,
	...
	/**
	 * 
	 * @return If the last mutation was an improvement 
	 * (e.g the childs fitness is better than the parents.) 
	 */
